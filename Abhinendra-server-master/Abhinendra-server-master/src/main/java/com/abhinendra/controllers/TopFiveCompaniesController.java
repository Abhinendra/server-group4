...
public class TopFiveCompaniesController {
    ...
    @GetMapping(value = "/display/{sectorName}", produces = "application/json")
    List<TopFiveDisplay> getTopFiveCompaniesOfSectorDisplay(@PathVariable("sectorName") String sectorName) {
        String[] Food = {"ADFFOODS", "DFMFOODS", "ATFL", "DAAWAT", "HERITGFOOD", "KOHINOOR", "LAKSHMIEFL"};
        String[] Mill = {"PHOENIXLTD", "APOLLO", "AMJLAND", "RAMGOPOLY", "ASTRON", "SHIVATEX"};
        String[] Software = {"SONATSOFTW", "TERASOFT", "CALSOFT", "COMPUSOFT", "CYBERTECH", "NEWGEN", "FCSSOFT"};
        String[] Bank = {"BANKBARODA", "BANKINDIA", "DENABANK", "SYNDIBANK", "CORPBANK", "UCOBANK", "VIJAYABANK",
        ...
        ArrayList<TopFiveDisplay> listFinal=new ArrayList<>();

        for(TopFive topFive:list.subList(0, 5)) {
            TopFiveDisplay topd1=companyDetailLoader.loadDetailsDaily(topFive.getDatacode(),topFive.getGains());
            listFinal.add(topd1);
